import { ticketRef, LABELS, agents } from './db';
...
// AGENT_SLACK_IDS maps agent names to Slack member IDs so mentions trigger
// real notifications, e.g. "Vitalijs:U05U0RP8QQ1,Ella:U0B771G4DGB"
function idMap() {
...
function mentionAllAgents() {
  return agents().map(mention).join(' ');
}
...
export async function notifyAssigned(t, assignee) {
  const link = appUrl(`/tickets/${t.id}`);
  await notifySlack(
    `:bust_in_silhouette: *${ticketRef(t.id)}* assigned to ${mention(assignee)} — ${t.subject}` +
      (link ? `\n<${link}|Open ticket>` : '')
  );
}
